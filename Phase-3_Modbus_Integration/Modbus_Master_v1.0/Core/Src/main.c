...
int main(void)
{
  ...
  MX_USART2_UART_Init();
  /* USER CODE BEGIN 2 */
  UARTx_Cell_Comm_Initialization();

  HAL_TIM_Base_Start_IT(&htim2);
  Tick_Timer_Initialization(&message1sTime, 1000);		// 1 sn
  ...
  while (1)
  {
    ...
    /* USER CODE BEGIN 3 */
	  ...
	  {
		  Tick_Timer_Set_Time(&message1sTime, 1000);
	  }

	  if(HAL_GPIO_ReadPin(GPIOA, GPIO_PIN_0))
	  {
		  HAL_Delay(500);
		  Modbus_Read_Input_Registers(&modbus, 5, 0, 5);
	  }
